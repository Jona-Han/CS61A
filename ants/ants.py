...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_location is not beehive:
            if current_location.bees and self.min_range <= range_checker <= self.max_range:
                return rANTdom_else_none(current_location.bees)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if amount >= self.armor:
            armor_loss = self.damage + amount
        else:
            armor_loss = amount
        for bee in list(self.place.bees):
            bee.reduce_armor(armor_loss)
...
# BEGIN Problem 7
class WallAnt(Ant):
    """WallAnt does no action each turn"""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        # BEGIN Problem EC
        if not QueenAnt.already_queen:
            QueenAnt.already_queen = True
    ...
    def action(self, gamestate):
        ...
        if not self.true_queen:
            ...
            while current_location != None:
                if current_location.ant and not current_location.ant.buffed:
                    current_location.ant.buffed = True
                    current_location.ant.damage *= 2
